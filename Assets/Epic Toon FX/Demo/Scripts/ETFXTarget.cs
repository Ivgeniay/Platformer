...
public class ETFXTarget : MonoBehaviour
{
    ...
    void SpawnTarget()
    {
		...
		targetCollider.enabled = true; //Enables the collider_
    ...
    {
        if (col.tag == "Missile") // If collider_ is tagged as missile
        {
            if (hitParticle)
            {
				...
				targetCollider.enabled = false; // Disables target collider_
